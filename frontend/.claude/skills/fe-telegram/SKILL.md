...
Нужно знать тему в самом коде (другая картинка под тёмную) — добавь в
`shared/telegram` хук над `miniApp.isDark`; для цветов это не требуется.

### Панель разработки в браузере

В dev-сборке с `VITE_MOCK_TELEGRAM=1` в правом верхнем углу есть панель
(`app/dev/DevPanel.tsx`): переключатель языка RU / BE / EN и переключатель темы:
...
Код: `shared/telegram/theme-mock.ts` (схемы и событие) и `app/dev/` (кнопки).
Внутри Telegram панели нет: тему выбирает пользователь в телефоне, язык приходит
из его настроек Telegram.

### Внешние ссылки

Только `openExternalLink` из `@/shared/telegram`, не `window.open` и не `<a target>`:
мини-апп при этом не выгружается и пользователь возвращается на тот же экран.
