...
```bash
cd frontend
ls src/shared/lib src/shared/ui src/shared/telegram
...
## Правило первого потребителя

**Утилита, хук, тип и компонент появляются вместе с местом, которое их вызывает.**
Не создавай их «пригодятся»: неиспользуемый код никто не удалит, но каждый будет
на него натыкаться, а к моменту реального применения он окажется неподходящим.

Исключение — обёртки над платформой (`shared/telegram`, `shared/api`). Они могут
существовать заранее, потому что их задача обратная: не пустить SDK и `fetch`
в остальной код. Про запас — нельзя, вместо чего-то — можно.

То же про папки: `shared/hooks` и `shared/types` заводятся первым файлом,
а не заранее пустыми.

## Куда положить новый код

| Что                                      | Куда                        | Пример                           |
| ---------------------------------------- | --------------------------- | -------------------------------- |
| Переиспользуемый компонент без домена    | `shared/ui/<Имя>.tsx`       | `EmptyState`, `ErrorState`       |
...
| Переиспользуемый хук                     | `shared/hooks/use-<имя>.ts` | папка появляется с первым хуком  |
...
| Общие типы-утилиты                       | `shared/types/`             | папка появляется с первым типом  |
...
- `shared/types/` — типы-утилиты без предметного смысла (`Nullable<T>` и подобные).
...
- Барели внутри сегментов: `types/index.ts`, `api/index.ts` в слайсе не нужны —
  публичный API один, это `index.ts` слайса.
- Файл под одну трёхстрочную функцию, если рядом есть естественный дом.
