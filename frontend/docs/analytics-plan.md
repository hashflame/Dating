...
Статус: **реализовано** (истории 1–5). Рабочая справка — [`analytics.md`](analytics.md);
здесь остались рассуждения, откуда взялись решения. Расхождения с планом,
случившиеся при реализации, перечислены в `analytics.md` (раздел «Чего в
событиях нет и почему»).
