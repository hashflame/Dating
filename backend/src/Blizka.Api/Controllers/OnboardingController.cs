...
public sealed class OnboardingController(IMediator mediator) : ControllerBase
{
    ...
    /// <summary>
    /// Удаляет черновик онбординга текущего пользователя, возвращает его <c>Status</c> в <c>New</c> и очищает
    /// его собственные свайпы (лайки/дизлайки) — debug-утилита для повторного прогона регистрации и ленты на
    /// нестабильном стенде без создания нового Telegram-пользователя на каждый прогон. Не начисленные зорки/
    /// загруженные фото/интересы/мэтчи это не трогает — не полное удаление аккаунта.
    /// </summary>
    /// <response code="204">Черновик удалён (или его и не было), статус пользователя и свайпы сброшены.</response>
    /// <response code="401">Токен отсутствует или невалиден.</response>
    [HttpDelete("draft")]
