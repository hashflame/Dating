...
public sealed class ConfirmDateCommandHandlerTests
{
    [Fact(DisplayName = "КОГДА встреча ещё не подтверждена ТОГДА хендлер фиксирует DateConfirmedAt и сохраняет")]
    public async Task Handle_confirms_the_date_for_the_first_time()
    {
        ...
        var handler = new ConfirmDateCommandHandler(repository);

        await handler.Handle(new ConfirmDateCommand(match.Id, currentUser.Id), CancellationToken.None);

        Assert.NotNull(match.DateConfirmedAt);
        Assert.Equal(currentUser.Id, match.DateConfirmedByUserId);
        Assert.True(repository.SaveChangesCalled);
    }

    [Fact(DisplayName = "КОГДА встреча уже подтверждена ТОГДА повторный вызов не сдвигает DateConfirmedAt и не сохраняет заново")]
    public async Task Handle_is_idempotent_when_already_confirmed()
    {
        var currentUser = CreateUser();
        var other = CreateUser(name: "Anna");
        var match = CreateMatch(currentUser, other);
        var originalTimestamp = DateTimeOffset.UtcNow.AddDays(-1);
        match.DateConfirmedAt = originalTimestamp;
        match.DateConfirmedByUserId = other.Id;
        var repository = new FakeMatchRepository { ById = match };
        var handler = new ConfirmDateCommandHandler(repository);

        await handler.Handle(new ConfirmDateCommand(match.Id, currentUser.Id), CancellationToken.None);

        Assert.Equal(originalTimestamp, match.DateConfirmedAt);
        Assert.Equal(other.Id, match.DateConfirmedByUserId);
    ...
    public async Task Handle_throws_when_the_match_is_not_found_for_the_requesting_user()
    {
        ...
        await Assert.ThrowsAsync<MatchNotFoundException>(
            () => handler.Handle(new ConfirmDateCommand(Guid.NewGuid(), Guid.NewGuid()), CancellationToken.None));
    }
    ...
    private sealed class FakeMatchRepository : IMatchRepository
    {
        ...
        public Task AddAsync(Match match, CancellationToken cancellationToken) =>
            throw new NotSupportedException("Не используется в тестах подтверждения даты.");
